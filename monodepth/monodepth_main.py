...
def custom_test(params):
    ...
    for step in range(num_test_samples):
        ...
        city_name = os.path.dirname(left_paths[step])
        np.save(os.path.join(output_directory, city_name, "{}_disp.npy".format(output_name)), disp_pp)
        disp_to_img = scipy.misc.imresize(disp_pp.squeeze(), [1024, 2048])
        plt.imsave(os.path.join(output_directory, city_name, "{}_disp.png".format(output_name)), disp_to_img, cmap='plasma')
    print('done.')
